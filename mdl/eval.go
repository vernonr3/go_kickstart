...
func modelizeComponents(cs []*expr.Component) []*Component {
	...
	for i, c := range cs {
		res[i] = &Component{
			...
			Properties:    c.Properties,
			Relationships: modelizeRelationships(c.Relationships),
			Structs:       modelizeStructs(c.Structs),
			Interfaces:    modelizeInterfaces(c.Interfaces),
		}
	}
	return res
}

func modelizeStructs(cs []*expr.Struct) []*Struct {
	res := make([]*Struct, len(cs))
	for i, c := range cs {
		res[i] = &Struct{
			ID:            c.ID,
			Name:          c.Name,
			...
			Properties:    c.Properties,
			Relationships: modelizeRelationships(c.Relationships),
			Methods:       modelizeMethods(c.Methods),
		}
	}
	return res
}

func modelizeInterfaces(cs []*expr.Interface) []*Interface {
	res := make([]*Interface, len(cs))
	for i, c := range cs {
		res[i] = &Interface{
			ID:            c.ID,
			Name:          c.Name,
...
func modelizeMethods(cs []*expr.Method) []*Method {
